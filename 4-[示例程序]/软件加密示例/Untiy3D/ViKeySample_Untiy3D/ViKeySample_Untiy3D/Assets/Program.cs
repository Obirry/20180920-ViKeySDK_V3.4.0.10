...
namespace ViKeySample
{
    class Program
    {
		...
		public static void ViKeyTest()
        {
            UnityEngine.Debug.Log("********ViKey C# Sample********");
            ...
            if (retcode != 0)
            {
                UnityEngine.Debug.Log("查找ViKey加密狗错误 error code: "+ retcode);
                return;
            }
            ...
            for (j = 0; j < Count; j++)
            {
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("获取硬件序列号错误 error code: " + retcode);
                    return;
                }
                UnityEngine.Debug.Log("ViKey加密狗硬件序列号:" + HID);


                retcode = VikeyGetType(j, ref ViKeyType);
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("获取ViKey加密狗类型 error code:"+ retcode);
                    return;
                }
                UnityEngine.Debug.Log("获取ViKey加密狗类型:" + ViKeyType);
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("用户登陆ViKey加密狗失败 error code: "+ retcode);
                    return;
                }
                //管理员登陆加密狗
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("管理员登陆ViKey加密狗失败 error code: " + retcode);
                    return;
                }
...
//                 if (retcode != 0)
//                 {
                //                     UnityEngine.Debug.Log("设置设备名称失败 error code: "+ retcode);
//                 }
// 
//                 retcode =  VikeyGetPtroductName(j, szProductName);
//                 if (retcode != 0)
//                 {
                //                     UnityEngine.Debug.Log("获取设备名称失败 error code: "+ retcode);
//                 }
//                 else
//                 {
//                     UnityEngine.Debug.Log("获取设备名称成功: " + System.Text.Encoding.Unicode.GetString(szProductName));
//                 }
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("写数据失败 error code: "+retcode);
                    return;
                }
                UnityEngine.Debug.Log("Write:" + str1);
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("读数据失败 error code: "+ retcode);
                    return;
                }
                str1 = System.Text.Encoding.ASCII.GetString(buffer);
                UnityEngine.Debug.Log("Read:" + str1);

                // random generation function
                retcode = ViKeyRandom(j, ref data1, ref data2, ref data3, ref data4);
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("获取随机数失败 error code: "+ retcode);
                    return;
                }
                UnityEngine.Debug.Log("Random: " + data1);
...
//                 if (retcode != 0)
//                 {
                //                     UnityEngine.Debug.Log("获取种子失败 error code: "+ retcode);
//                     return;
//                 }
//                 UnityEngine.Debug.Log("Seed:" + data1 + " " + data2 + " " + data3 + " " + data4);
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("设置软件ID error code: "+ retcode);
                    return;
                }
                UnityEngine.Debug.Log("设置软件ID: " + strSoftIDString);
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("读取软件ID error code: "+ retcode);
                    return;
                }
                strSoftIDString = System.Text.Encoding.ASCII.GetString(buffer);
                UnityEngine.Debug.Log("读取软件ID: " + strSoftIDString);

                //时钟狗支持获取时间
                //retcode = VikeyGetTime(j, buffer);
                //if (retcode != 0)
                //{
                //    UnityEngine.Debug.Log("读取加密狗内部时间 error code: "+ retcode);
                //    return;
                //}
                //UnityEngine.Debug.Log("读取加密狗内部时间: " + buffer[0] + "\\" + buffer[1] + "\\" + buffer[2] + "\\ " +
                ...
                if (retcode != 0)
                {
                    UnityEngine.Debug.Log("注销ViKey加密狗 error code: "+ retcode);
                    return;
                }
            }
            //getch();
