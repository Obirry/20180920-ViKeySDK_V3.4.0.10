...
VikeyGetTime:function(Index:Word;pTime:pByteArray):DWORD; stdcall;
...
VikeyGetTime:=GetProcAddress(Handle,'VikeyGetTime');
...
//read dongle memory
 ...
 Addr:=0;			//[in] start address in Dongle Memory
  Length:=10;			//[in] number of byte to write
   ...
   retcode:= VikeyGetTime(wIndex,@tmpBuf[0]);
   if retcode<>0 then
   begin
        FmtStr(str,'testing Get Time , find error %d',[retcode]);
        List.Items.Add(str);

   end;                                                                     //,[mP2],[mP3],[mP4]
   FmtStr(str,'ViKey Get Time success! random : %d:, %d:, %d:, %d:, %d:, %d:',[tmpBuf[0] ,tmpBuf[1],tmpBuf[2],tmpBuf[3],tmpBuf[4],tmpBuf[5]]);
   List.Items.Add(str);
